...
def fetch_shot_events(team_name, match_ids):
    shot_events = []
    for match_id in match_ids:
        ...
        team_events = [event for event in match_events if event['team']['name'] == team_name and event['period'] in range(1, 5)]
        shot_events.extend([event for event in team_events if 'shot' in event])
    return shot_events

def plot_goal_shot_locations(team_name, shot_events):
    ...
    ax.set_title(f"{team_name} Tournament Summary\n{summary_str}", fontsize=12)
...
# Fetch match IDs and shot events
match_ids = fetch_match_ids(competition_id, season_id)
shot_events = fetch_shot_events(team_name, match_ids)

# Plot all shot locations with goals annotated
plot_goal_shot_locations(team_name, shot_events)
plt.show()
